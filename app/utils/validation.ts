...
export function validateAmount(amount: unknown): { valid: boolean; error?: string } {
  ...
  if (amount > MAX_AMOUNT) {
    return { valid: false, error: `금액은 ${MAX_AMOUNT.toLocaleString()}원을 초과할 수 없습니다` }
  }
 ...
 * 목표 비중 검증 (0-100%)
 */
export function validateTargetPercent(percent: unknown): { valid: boolean; error?: string } {
...
export function validateCategoryName(name: unknown): { valid: boolean; error?: string } {
  ...
  if (trimmed.length > MAX_CATEGORY_NAME_LENGTH) {
    return { valid: false, error: `카테고리 이름은 ${MAX_CATEGORY_NAME_LENGTH}자를 초과할 수 없습니다` }
  }
...
export function validateDescription(description: unknown): { valid: boolean; error?: string } {
  ...
  if (description.length > MAX_DESCRIPTION_LENGTH) {
    return { valid: false, error: `설명은 ${MAX_DESCRIPTION_LENGTH}자를 초과할 수 없습니다` }
  }
...
export function validateDate(date: unknown): { valid: boolean; error?: string } {
  ...
  // 입력된 년, 월, 일 파싱 (정규식 통과 후이므로 항상 3개 요소 존재)
 ...
 * 색상 코드 검증
 */
export function validateColor(color: unknown): { valid: boolean; error?: string } {
  ...
  if (!hexRegex.test(color)) {
    return { valid: false, error: '유효한 색상 코드가 아닙니다 (예: #FF0000)' }
  }
...
export function validateSavingInput(input: {
  ...
  } else if (typeof input.category_id !== 'number' || !Number.isInteger(input.category_id)) {
